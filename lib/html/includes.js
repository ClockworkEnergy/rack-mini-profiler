...
var _MiniProfiler = (function() {
  ...
  var onTurboBeforeVisit = function onTurboBeforeVisit(e) {
    if(!e.defaultPrevented) {
      ...
      window.MiniProfiler.pageTransition(visitingUrl)
    }
  }
  ...
  return {
    ...
    pageTransition: function pageTransition(visitingUrl) {
      if (totalsControl) {
      ...
      // Preserve the last prefetched profiling result so badge doesn't disappear when Turbo replaces page HTML
      if (visitingUrl === lastTurboRequestUrl && visitingUrl !== Turbo.session.view.lastRenderedLocation.href && lastTurboProfilingResult) {
